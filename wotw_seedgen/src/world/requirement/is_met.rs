...
use crate::util::orbs::{self, OrbVariants, Orbs};
    ...
    pub fn is_met(
        ...
        match self {
            ...
            Requirement::Difficulty(difficulty) => {
                if player.settings.difficulty >= *difficulty {
                    return orb_variants;
                }
            }
            ...
            Requirement::Trick(trick) => {
                if player.settings.tricks.contains(trick) {
                    return orb_variants;
                }
            }
            ...
            Requirement::SpiritLight(amount) => {
                if player.inventory.has(&Item::SpiritLight(1), *amount) {
                    return orb_variants;
                }
            }
            Requirement::Resource(resource, amount) => {
                if player.inventory.has(&Item::Resource(*resource), *amount) {
                    return orb_variants;
                }
            }
            Requirement::Shard(shard) => {
                if player.inventory.has_any(&Item::Shard(*shard)) {
                    return orb_variants;
                }
            }
            Requirement::Teleporter(teleporter) => {
                if player.inventory.has_any(&Item::Teleporter(*teleporter)) {
                    return orb_variants;
                }
            }
            Requirement::Water => {
                if player.inventory.has_any(&Item::Water) {
                    return orb_variants;
                }
            }
                ...
                let combat_is_met = || {
                    ...
                    for (enemy, amount) in enemies {
                        ...
                        match enemy {
                            ...
                            Enemy::Sandworm => {
                                if player.inventory.has_any(&Item::Skill(Skill::Burrow)) {
                                    continue;
            ...
            Requirement::ShurikenBreak(health) => {
                if player.inventory.has_any(&Item::Skill(Skill::Shuriken)) {
                    let clip_mod = if player.settings.difficulty >= Difficulty::Unsafe {
                        2.0
                    } else {
                        3.0
                    };
...
fn cost_is_met(
    ...
    fn orbs_meet_cost(
        ...
        let has_life_pact = player.settings.difficulty >= logical_difficulty::LIFE_PACT
            && player.inventory.has_any(&Item::Shard(Shard::LifePact));
...
fn health_is_met(
    ...
    orb_variants.retain(|orbs| {
        let met = orbs.health > cost
            || (player.inventory.has_any(&Item::Skill(Skill::Regenerate))
