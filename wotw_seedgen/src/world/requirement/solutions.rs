...
impl Requirement {
    ...
    pub fn solutions(&self, player: &Player, states: &FxHashSet<usize>, orb_variants: OrbVariants, slots: usize, world_slots: usize) -> Vec<Inventory> {
        let (player_item_count, player_spirit_light) = item_count_and_spirit_light(&player.inventory);

        // We only need to clone this in debug builds where we want to display it later when some checks fail
        #[cfg(debug_assertions)] let cloned_if_debug = orb_variants.clone();
        #[cfg(not(debug_assertions))] let cloned_if_debug = orb_variants;
        ...
        #[cfg(debug_assertions)] self.check_solutions(&solutions, player, states, orb_variants);

        solutions
    }

    fn find_solutions(&self, solutions: &mut Vec<TaggedSolution>, player: &Player, states: &FxHashSet<usize>, slots: u32, world_slots: u32, player_item_count: u32, player_spirit_light: u32) {
        match self {
            Requirement::Free => {},
            Requirement::Impossible | Requirement::Difficulty(_) | Requirement::NormalGameDifficulty | Requirement::Trick(_) => solutions.clear(),
            Requirement::Skill(skill) => require(solutions, Item::Skill(*skill)),
            Requirement::EnergySkill(skill, amount) => needed_for_energy_skill(solutions, player, *skill, *amount, true),
            Requirement::NonConsumingEnergySkill(skill) => needed_for_energy_skill(solutions, player, *skill, 1.0, false),
            Requirement::SpiritLight(amount) => require_missing(solutions, Item::SpiritLight(1), *amount),
            Requirement::Resource(resource, amount) => require_missing(solutions, Item::Resource(*resource), *amount),
            Requirement::Shard(shard) => require(solutions, Item::Shard(*shard)),
            Requirement::Teleporter(teleporter) => require(solutions, Item::Teleporter(*teleporter)),
            Requirement::Water => require(solutions, Item::Water),
            Requirement::State(state) => if !states.contains(state) { solutions.clear() }
            Requirement::Damage(amount) => needed_for_health(solutions, all(solutions), *amount, player, true),
            Requirement::Danger(amount) => needed_for_health(solutions, all(solutions), *amount, player, false),
            ...
            Requirement::ShurikenBreak(health) => needed_to_destroy_with(solutions, Skill::Shuriken, if player.settings.difficulty >= Difficulty::Unsafe { 2.0 } else { 3.0 }, player, *health),
            Requirement::SentryBreak(health) => needed_to_destroy_with(solutions, Skill::Sentry, 6.25, player, *health),
            Requirement::Combat(enemies) => needed_for_combat(solutions, player, enemies),
            Requirement::And(ands) => ands.iter().for_each(|and| and.find_solutions(solutions, player, states, slots, world_slots, player_item_count, player_spirit_light)),
            Requirement::Or(ors) => alternate_solutions(solutions, ors, |solutions, or| or.find_solutions(solutions, player, states, slots, world_slots, player_item_count, player_spirit_light)),
        }
        check_slot_limits(solutions, slots, world_slots, player_item_count, player_spirit_light);
    }

    fn check_solutions(&self, solutions: &[Inventory], player: &Player, states: &FxHashSet<usize>, orb_variants: OrbVariants) {
        for solution in solutions {
            ...
            let solution_orb_variants = orb_variants.iter().map(|orbs| *orbs + solution_extra_orbs).collect();
            let is_met = !self.is_met(&solution_player, states, solution_orb_variants).is_empty();
...
fn duplicate_solutions(total_solutions: &mut Vec<TaggedSolution>, solutions: Vec<usize>, tag: Tag) -> (Vec<usize>, Vec<usize>)
{
    ...
    let (using, mut not_using) = solutions.into_iter().partition::<Vec<_>, _>(|index| {
        total_solutions[*index].tags[tag as usize].unwrap_or_else(|| {  // If this tag hasn't been applied yet, we have to duplicate this solution to start tracking both variants and it shouldn't cause redundancies
...
fn call_for_alternatives<T, F>(total_solutions: &mut Vec<TaggedSolution>, solutions: Vec<usize>, player: &Player, tag: Tag, value_when_using: T, value_when_not_using: T, f: F)
...
fn all(solutions: &[TaggedSolution]) -> Vec<usize> { (0..solutions.len()).collect() }
...
{
    *solutions = alternatives.into_iter().flat_map(|alternative| {
...
fn require(solutions: &mut [TaggedSolution], item: Item) {
    solutions.iter_mut().for_each(|solution| solution.inventory.grant(item.clone(), 1));
}
fn require_missing(solutions: &mut [TaggedSolution], item: Item, amount: u32) {
    solutions.iter_mut().for_each(|solution| {
        ...
        solution.inventory.grant(item.clone(), amount.saturating_sub(has));
    });
}
...
fn needed_for_health(total_solutions: &mut Vec<TaggedSolution>, solutions: Vec<usize>, mut amount: f32, player: &Player, consuming: bool) {
    let game_thinks_regenerate_cost = Skill::Regenerate.energy_cost();
    let mut regenerate_cost = game_thinks_regenerate_cost;
    if player.settings.difficulty < Difficulty::Unsafe { regenerate_cost *= 2.0; }

    if player.settings.hard { amount *= 2.0 }

    call_for_alternatives(total_solutions, solutions, player, Tag::UseResilience, 0.9, 1.0, |total_solutions, solutions, defense_mod| {
        ...
        let health_fragments_solution = |mut orbs: Orbs| {
            ...
            if consuming { orbs.health -= amount }
            (missing_health_fragments as u32, orbs)
        };

        call_for_alternatives(total_solutions, solutions, player, Tag::UseOvercharge, 0.5, 1.0, |total_solutions, solutions, cost_mod| {  // We are accepting some redundancies here if the player doesn't actually need to heal
            ...
            for index in solutions {
                ...
                let (missing_health_fragments, orbs) = health_fragments_solution(solution.orbs);
                ...
                let can_regen = solution.orbs.energy >= higher_cost || solution.health_payed_for_life_pact == 0.0;
                if can_regen {  // We don't use the tag here because in this context Regenerate never has a strict advantage, we'd never to cover all cases anyway
                    let max_regens = ((max_health - solution.orbs.health) * (1.0 / 30.0)).ceil() as u32;
                    if max_regens > 0 {
                        ...
                        for _ in 1..=max_regens {
                            ...
                            total_missing_energy_fragments += missing_energy_fragments as u32;
                            solution.orbs.energy += missing_energy_fragments * 0.5 - regenerate_cost;  // granting fragments increases max energy as well
                            solution.orbs.health = (solution.orbs.health + 30.0).min(max_health);
...
fn needed_for_energy(total_solutions: &mut Vec<TaggedSolution>, solutions: Vec<usize>, mut cost: f32, player: &Player, consuming: bool) {
    if player.settings.difficulty < Difficulty::Unsafe { cost *= 2.0 }

    call_for_alternatives(total_solutions, solutions, player, Tag::UseOvercharge, 0.5, 1.0, |total_solutions, solutions, cost_mod| {
        let cost = cost * cost_mod;

        call_for_alternatives(total_solutions, solutions, player, Tag::UseLifePact, true, false, |total_solutions, solutions, use_life_pact| {  // We are accepting some redundancies here if the player doesn't actually need Life Pact
            if use_life_pact {
                let difficulty_mod = if player.settings.hard { 2.0 } else { 1.0 };
                call_for_alternatives(total_solutions, solutions, player, Tag::UseResilience, 0.9 * difficulty_mod, difficulty_mod, |total_solutions, mut solutions, defense_mod| {
                    let f = |total_solutions: &mut Vec<TaggedSolution>, solutions: Vec<usize>, use_regenerate| {
                        for index in solutions {
                            ...
                            if use_regenerate {
                                ...
                                let game_thinks_regenerate_cost = Skill::Regenerate.energy_cost();
                                let mut regenerate_cost = game_thinks_regenerate_cost * cost_mod;
                                if player.settings.difficulty < Difficulty::Unsafe { regenerate_cost *= 2.0; }
                                let higher_cost = regenerate_cost.max(game_thinks_regenerate_cost);
                                ...
                                let game_thinks_total_cost = total_cost - regenerate_cost + higher_cost;  // On the final regenerate we have to make sure the the game is happy with our amount of resources
                                grant_energy(solution, total_cost, game_thinks_total_cost);
                                ...
                                if max_heal > regenerate_cost * 10.0 {  // oh no we found a suboptimal heal that we may want to do
                                    let old_solution = solution.clone();
                                    debug_assert!(solution.orbs.health > max_health - 30.0);

                                    grant_energy(solution, regenerate_cost, higher_cost);
                            ...
                            for index in iter::once(index).chain(new_index) {
                                ...
                                let max_energy = solution.inventory.max_energy(player.settings.difficulty);
                                ...
                                loop {
                                    if missing_energy <= 0.0 {
                                        ...
                                        health_fragment_solution(&mut solution, health_cost);
                                        solution.do_not_generate_all_life_pact_variants = true;  // This is a note to future requirements so we can generate the health/energy fragment combinations for chained requirements with less redundancies
                    ...
                    let regen_is_worth_it = !(player.settings.hard && player.settings.difficulty < Difficulty::Unsafe);
                    let never_regen = if regen_is_worth_it {
                        ...
                        let partition_index = partition::partition_index(&mut solutions, |index| {  // TODO use partition_in_place once stabilized
                            let solution = &total_solutions[*index];
                            ...
                            if solution.health_payed_for_life_pact > 0.0 { return false }

                            // TODO we do this calculation again later... should we store it somewhere?
                            let max_health = solution.inventory.max_health(player.settings.difficulty);
                            let max_heal = max_health - solution.orbs.health - solution.health_payed_for_life_pact;  // In order to regenerate, we will have to grant energy fragments until we undid all the health payed for life pact

                            let mut regenerate_cost = Skill::Regenerate.energy_cost() * cost_mod;
                            if player.settings.difficulty < Difficulty::Unsafe { regenerate_cost *= 2.0; }

                            max_heal > regenerate_cost * 10.0
                        });
                        let never_regen = solutions.split_off(partition_index);
                        call_for_alternatives(total_solutions, solutions, player, Tag::UseRegenerate, true, false, f);
                        never_regen
                    } else { solutions };
...
fn needed_for_energy_skill(solutions: &mut Vec<TaggedSolution>, player: &Player, skill: Skill, amount: f32, consuming: bool) {
    require(solutions, Item::Skill(skill));
    needed_for_energy(solutions, all(solutions), skill.energy_cost() * amount, player, consuming);  // TODO because of the game miscalculating use costs, it's slightly incorrect to just multiply the cost here
}
fn needed_to_destroy<const TARGET_IS_WALL: bool>(solutions: &mut Vec<TaggedSolution>, player: &Player, health: f32) {
    alternate_solutions(solutions, player.progression_weapons::<TARGET_IS_WALL>(), |solutions, weapon| needed_to_destroy_with(solutions, weapon, 1.0, player, health));
}
fn needed_to_destroy_with(solutions: &mut Vec<TaggedSolution>, weapon: Skill, cost_mod: f32, player: &Player, health: f32) {
...
fn destroy_cost(weapon: Skill, player: &Player, health: f32, flying_target: bool) -> f32 {
    ...
    let damage = weapon.damage(player.settings) * player.damage_mod(flying_target, matches!(weapon, Skill::Bow)) + weapon.burn_damage();  // We are only allowed to add the player's damage mod here because we're not factoring damage upgrades into the progressions yet
    weapon.energy_cost() * (health / damage).ceil()
}

fn needed_for_combat(solutions: &mut Vec<TaggedSolution>, player: &Player, enemies: &[(Enemy, u8)]) {
    ...
    let mut weapon_combinations = Vec::with_capacity(weapons_len * ranged_weapons.len() * shield_weapons.len());
    for ranged_weapon in ranged_weapons {
        for &shield_weapon in &shield_weapons {  // TODO I think this is meant to filter overlap between ranged and shield weapons as well
    ...
    alternate_solutions(solutions, weapon_combinations, |solutions, (weapon, ranged_weapon, shield_weapon, burrow)| {
        let mut cost = 0.0;
        ...
        for (enemy, amount) in enemies {
            ...
            match enemy {
                Enemy::EnergyRefill => {
                    needed_for_energy(solutions, all(solutions), cost, player, true);
                    solutions.iter_mut().for_each(|solution| solution.inventory.recharge(&mut solution.orbs, amount, player.settings.difficulty));
                    cost = 0.0;
                    continue;
                },
        ...
        needed_for_energy(solutions, all(solutions), cost, player, true);
        if melee { require(solutions, Item::Skill(weapon)); }
        if ranged { require(solutions, Item::Skill(ranged_weapon)); }
        if shielded { require(solutions, Item::Skill(shield_weapon)); }
        if burrow { require(solutions, Item::Skill(Skill::Burrow)); }
    });

    if player.settings.difficulty < Difficulty::Unsafe {
        if bash { require(solutions, Item::Skill(Skill::Bash)); }
...
fn item_count_and_spirit_light(inventory: &Inventory) -> (u32, u32) {
    ...
    let item_count = inventory.items.iter()
        .filter_map(|(item, amount)| match item {
            Item::SpiritLight(stacked_amount) => { spirit_light += *amount * *stacked_amount; None },
            _ => Some(*amount),
        }).sum::<u32>();
    (item_count, spirit_light)
}
fn check_slot_limits(solutions: &mut Vec<TaggedSolution>, slots: u32, world_slots: u32, player_item_count: u32, player_spirit_light: u32) {
    solutions.retain(|solution| {
        ...
        item_count - player_item_count + spirit_light_slots <= slots && spirit_light_slots <= world_slots  // Spirit light is the only world item currently
    });
}
