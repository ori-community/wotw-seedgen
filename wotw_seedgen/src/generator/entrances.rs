...
impl WorldContext<'_, '_> {
    pub fn generate_entrances(&mut self) -> Result<(), String> {
        let (connections, loop_size) = match self.world.settings.randomize_entrances {
            None => {
                log::trace!("Using default entrance connections");
                (self.world.graph.default_entrance_connections.clone(), 2)
            }
            Some(loop_size) => {
                ...
                self.output.events.extend((1..=32).map(|entrance_id| Event {
                    trigger: Trigger::ClientEvent(ClientEvent::Spawn),
                ...
                let connections = self.generate_entrance_connections(&config)?;
    ...
    fn generate_entrance_connections(
        ...
        let final_state = self.generate_entrance_connections_recursively(&initial_state, config)?;
    ...
    fn generate_entrance_connections_recursively(
        ...
        for possible_target_entrance in possible_target_entrances {
            ...
            let target_entrance_group_index =
                config.group_index_by_entrance_id[&target_entrance_id];
            ...
            if state.current_loop_start.unwrap() == target_entrance_id {
                ...
                for possible_next_entrance_id in possible_next_entrances {
                    ...
                    if let Ok(state) =
                        self.generate_entrance_connections_recursively(&state, config)
                    {
                ...
                if let Ok(state) = self.generate_entrance_connections_recursively(&state, config) {
                    log::trace!("{log_indent}Worked! {entrance_id} → {target_entrance_id}");
                    return Ok(state);
                }
