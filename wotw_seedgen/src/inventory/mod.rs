...
use crate::{
    item::{Item, Resource, Shard, Skill},
    log,
    settings::{logical_difficulty, Difficulty, WorldSettings},
    util::Orbs,
};
...
impl Inventory {
    pub fn grant(&mut self, mut item: Item, mut amount: u32) {
        ...
        if single_instance && amount > 1 {
            log::warning!(
                "Granted {} more than once, but that item can only be aquired once...",
                item
            );
        }
    ...
    pub fn cost(&self) -> u32 {
        self.items
            .iter()
            .map(|(item, amount)| item.cost() * *amount)
            .sum()
    }
    ...
    pub fn contains(&self, other: &Inventory) -> bool {
        ...
        other
            .items
            .iter()
            .all(|(item, amount)| self.has(item, *amount))
    }
    ...
    pub fn max_health(&self, difficulty: Difficulty) -> f32 {
        ...
        if difficulty >= logical_difficulty::VITALITY && self.has_any(&Item::Shard(Shard::Vitality))
        {
            health += 10.0;
        }
        health
    ...
    pub fn max_energy(&self, difficulty: Difficulty) -> f32 {
        ...
        if difficulty >= logical_difficulty::ENERGY_SHARD
            && self.has_any(&Item::Shard(Shard::Energy))
        {
            energy += 1.0;
        }
        energy
    ...
    pub fn damage_mod(&self, flying_target: bool, bow: bool, settings: &WorldSettings) -> f32 {
        ...
        if settings.difficulty >= logical_difficulty::DAMAGE_BUFFS {
            if self.has_any(&Item::Skill(Skill::GladesAncestralLight)) {
                damage_mod += 0.25;
            }
            if self.has_any(&Item::Skill(Skill::InkwaterAncestralLight)) {
                damage_mod += 0.25;
            }
            ...
            if flying_target && slots > 0 && self.has_any(&Item::Shard(Shard::Wingclip)) {
                damage_mod += 1.0;
                slots -= 1;
            }
            if slots > 0 && bow && self.has_any(&Item::Shard(Shard::Splinter)) {
                splinter = true;
                slots -= 1;
            }
            if slots > 0 && self.has_any(&Item::Shard(Shard::SpiritSurge)) {
                damage_mod += (self.get(&Item::SpiritLight(1)) / 10000) as f32;
                slots -= 1;
            }
            if slots > 0 && self.has_any(&Item::Shard(Shard::LastStand)) {
                damage_mod += 0.2;
                slots -= 1;
            }
            if slots > 0 && self.has_any(&Item::Shard(Shard::Reckless)) {
                damage_mod += 0.15;
                slots -= 1;
            }
            if slots > 0 && self.has_any(&Item::Shard(Shard::Lifeforce)) {
                damage_mod += 0.1;
                slots -= 1;
            }
            if slots > 0 && self.has_any(&Item::Shard(Shard::Finesse)) {
                damage_mod += 0.05;
            }
            if splinter {
                damage_mod *= 1.5;
    ...
    pub fn energy_mod(&self, settings: &WorldSettings) -> f32 {
        ...
        if settings.difficulty < Difficulty::Unsafe {
            energy_mod *= 2.0;
        } else if self.has_any(&Item::Shard(Shard::Overcharge)) {
            energy_mod *= 0.5;
        }
        energy_mod
    }
    pub fn defense_mod(&self, settings: &WorldSettings) -> f32 {
        let mut defense_mod = if settings.difficulty >= logical_difficulty::RESILIENCE
            && self.has_any(&Item::Shard(Shard::Resilience))
        {
            0.9
        } else {
            1.0
