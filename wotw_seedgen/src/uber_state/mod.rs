...
use crate::header::{parser, vdisplay, CodeDisplay, VResolve};
...
impl Display for UberIdentifier {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        rando_data::NAMED_UBER_STATES
            .iter()
            .find(|(_, identifier)| self == identifier)
            .map_or_else(
                || {
                    game_data::UBER_STATES
                        .iter()
                        .find(|(_, identifier)| self == identifier)
...
impl UberStateCondition {
    ...
    pub fn functionally_eq(&self, other: &Self) -> bool {
        match (&self.comparator, &other.comparator) {
            (UberStateComparator::Greater, UberStateComparator::GreaterOrEquals)
            | (UberStateComparator::LessOrEquals, UberStateComparator::Less) => {
                other.value.saturating_sub(self.value) == 1
            }
            (UberStateComparator::GreaterOrEquals, UberStateComparator::Greater)
            | (UberStateComparator::Less, UberStateComparator::LessOrEquals) => {
...
impl FromStr for UberStateTrigger {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        ...
        let item = VUberStateTrigger::parse(&mut parser)
...
impl UberStateTrigger {
    ...
    pub(crate) fn check_value(&self, value: f32) -> bool {
        self.condition
            .as_ref()
            .map_or(true, |condition| condition.met_by(value))
    }
        ...
        self.identifier == other.identifier
            && match &self.condition {
                Some(condition) => other
                    .condition
                    .as_ref()
                    .map_or(false, |other| condition.functionally_eq(other)),
    ...
    pub(crate) fn set_value(&self) -> u32 {
        self.condition
            .as_ref()
            .map_or(1, |condition| condition.value)
    }
}
