...
pub fn reach_check(mut args: cli::ReachCheckArgs) -> Result<(), String> {
    log_init::initialize_log(Some("reach.log"), LevelFilter::Off, false)
    ...
    let contents =
        fs::read_to_string(&args.seed_file).map_err(|err| format!("Error reading seed: {err}"))?;

    let universe_settings = UniverseSettings::from_seed(&contents)
        .unwrap_or_else(|| Err("No settings found in seed".into()))
    ...
    for item in args.items {
        match item {
            ...
            cli::ReachData::Teleporter(teleporter) => {
                inventory.grant(Item::Teleporter(teleporter), 1)
            }
